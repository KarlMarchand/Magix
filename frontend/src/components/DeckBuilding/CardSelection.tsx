import React, { useState } from "react";
import { useDeckManager } from "@context/DeckManagerContext/DeckManagerContext";
import GameCard from "@components/GameCard/GameCard";
import CardType from "@customTypes/Card";

interface CardSelectionProps extends React.HTMLProps<HTMLDivElement> {
	card: CardType;
}

const CardSelection: React.FC<CardSelectionProps> = ({ card, className, ...props }) => {
	const { currentDeck, addCard, removeCard } = useDeckManager();
	const [quantity, setQuantity] = useState<number>(currentDeck.cards[card.id] ?? 0);

	const changeQty = (modifier: number) => {
		let isChangedApplied = modifier > 0 ? addCard(card).isSuccessful : removeCard(card).isSuccessful;
		if (isChangedApplied) {
			setQuantity((current) => current + modifier);
		}
	};

	return (
		<div
			className={`d-flex flex-column align-items-center justify-content-center ${className ? className : ""}`}
			{...props}
		>
			<GameCard card={card} />
			{card.id != 1 && (
				<div className="d-flex justify-content-center align-items-center w-100">
					<button
						className="custom-btn px-3 py-1"
						onClick={() => changeQty(-1)}
						disabled={currentDeck.cards[card.id] === 0 || !currentDeck.cards[card.id]}
					>
						-
					</button>
					<span className="mx-3">{quantity}</span>
					<button
						className="custom-btn px-3 py-1"
						onClick={() => changeQty(1)}
						disabled={currentDeck.cards[card.id] === 3 || currentDeck.cardNumber === 30}
					>
						+
					</button>
				</div>
			)}
		</div>
	);
};

export default CardSelection;
